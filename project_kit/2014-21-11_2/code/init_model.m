...
%feat select
model.good_features = select_features(data,750);
data1 = data(:,model.good_features);
train_set_x = data1;
...
rem_feat(sum_data >= 750) = 1;

model.remove_features = rem_feat;

data1 = data1(:, (sum_data >= 750));
data1 = sparse(data1);
data = [data1];
train_set_x = data;
train_set_c = city_train;
...
for i = 1:7
    ...
    %disp 'training'
    city_indices = find(train_set_c(:,1) == 1);
    city_x = train_set_x(city_indices,:);
    city_y = train_set_y(city_indices);
