...
nb.p_y = sum(Y(Y==1))/length(Y);% YOUR CODE GOES HERE
nb.mu_x_given_y(:,1) = mean(X(find(Y==0),:)',2);                                
nb.mu_x_given_y(:,2) = mean(X(find(Y==1),:)',2);
X(find(Y==0),:)=bsxfun(@minus, X(find(Y==0),:), (nb.mu_x_given_y(:,1))');       
X(find(Y==1),:)=bsxfun(@minus, X(find(Y==1),:), (nb.mu_x_given_y(:,2))');
nb.sigma_x = std(X,1)';  
